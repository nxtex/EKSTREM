                  ...
                  <Card className="absolute top-20 left-1/2 -translate-x-1/2 w-64 bg-black/90 backdrop-blur-lg border-white/30 shadow-xl shadow-white/10 overflow-visible">
                    ...
                    <CardContent className="text-xs text-white/80">
                      ...
                      {item.relatedIds.length > 0 && (
                        <div className="mt-4 pt-3 border-t border-white/10">
                          <p className="text-white/50 text-xs mb-2 uppercase tracking-wider">Connected Nodes</p>
                          <div className="flex flex-wrap gap-1">
                            {item.relatedIds.map((relId) => {
                              const relItem = timelineData.find((d) => d.id === relId);
                              return relItem ? (
                                <Button
                                  key={relId}
                                  variant="outline"
                                  size="sm"
                                  className="h-6 text-xs px-2 border-white/20 text-white/70 hover:text-white hover:border-white/50 bg-transparent"
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    toggleItem(relId);
                                  }}
